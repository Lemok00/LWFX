...
style_img_path = './dataset/style009.jpg'
...
style_img = torch.from_numpy(style_img).type(torch.FloatTensor).unsqueeze(0).cuda()
content_img = torch.from_numpy(content_img).type(torch.FloatTensor).unsqueeze(0).cuda()
...
output = run_style_transfer(cnn, cnn_normalization_mean, cnn_normalization_std,
                            ...
                            num_steps=400,style_weight=800000,content_weight=1)
output_img = np.array(output.cpu().data)[0] * 255.0
...
output_img.save('./result/result087.png')
