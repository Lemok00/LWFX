...
if __name__ == '__main__':
    ...
    # init model
    model = nn.Sequential(alexnet(pretrained=True),nn.Linear(1000, 3), nn.Softmax())
    model.train()
    model.cuda()
    ...
    for epoch_idx in range(1, EPOCHS + 1):
        ...
        if epoch_idx % SAVE_EPOCH == 0:
            torch.save(model.state_dict(), 'model/0315_alexnet_{}.pth'.format(epoch_idx))
    ...
    for epoch_idx in range(1, 11):
        ...
        # save_model
        if epoch_idx % 1 == 0:
            torch.save(model.state_dict(), 'model/0315_alexnet_b_{}.pth'.format(epoch_idx))
