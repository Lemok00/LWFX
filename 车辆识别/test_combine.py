...
from torchvision.models import densenet121,resnet50,vgg16,densenet201,vgg19
from torchvision.transforms import transforms
from torchvision.datasets import ImageFolder

if __name__ == '__main__':
    TEST_DATA_PATH = 'dataset/test/'
    RESULT_PATH = 'result/result_0314_combine05.csv'
    BATCH_SIZE = 16
    ...
    temp_result=np.zeros(shape=(len(test_dataset),4,3))
    ...
    model = nn.Sequential(resnet50(pretrained=True),
                          ...
                          nn.Linear(1000, 3), nn.Softmax())
    model.load_state_dict(torch.load('model/0309_resnet_b_8.pth'))
    model.eval()
    model.cuda()
    # batch-loop
    for batch_idx, data in enumerate(test_loader, start=1):
    ...
    model.load_state_dict(torch.load('model/0310_vgg16_b_7.pth'))
    model.eval()
    model.cuda()
    # batch-loop
    for batch_idx, data in enumerate(test_loader, start=1):
        ...
        # model forward
        y = model(img).cpu().data.numpy()
        temp_result[BATCH_SIZE*(batch_idx-1):BATCH_SIZE*(batch_idx-1)+len(img),2,:]=y
    torch.cuda.empty_cache()

    # init model
    # model = CarDetectNetwork()
    model = nn.Sequential(densenet201(pretrained=True),nn.Linear(1000, 3), nn.Softmax())
    model.load_state_dict(torch.load('model/0310_densenet201_40.pth'))
    model.eval()
    model.cuda()
    # batch-loop
    for batch_idx, data in enumerate(test_loader, start=1):
        ...
        # model forward
        y = model(img).cpu().data.numpy()
        temp_result[BATCH_SIZE*(batch_idx-1):BATCH_SIZE*(batch_idx-1)+len(img),3,:]=y
    torch.cuda.empty_cache()
